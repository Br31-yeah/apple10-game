...
ICON_IMAGES = {}
for i in range(1, 10):
    path = f"{i}.png"  # 1.png, 2.png, ..., 9.png
    try:
        img = pygame.image.load(path)
        ICON_IMAGES[i] = pygame.transform.scale(img, (CELL_SIZE, CELL_SIZE))
    except:
        ICON_IMAGES[i] = None  # 없으면 기본 빨간색 사용
...
while running:
    ...
    for r in range(ROWS):
        for c in range(COLS):
            ...
            if num > 0:
                if ICON_IMAGES[num]:
                    SCREEN.blit(ICON_IMAGES[num], (x, y))
                else:
            ...
            if num > 0 and not ICON_IMAGES[num]:
